				...
				if( data[obj.name] ) {
					...
					$( '.attendant' ).each(function(index){
						...
						arr.push({
							...
							phone: $(this).find('.attendant-phone').val(),
							offline_message: $(this).find('.attendant-offline_message').val(),
							interval_message: $(this).find('.attendant-interval_message').val(),
							image: imageObj,
							default_timezone: $(this).find('.attendant-default-timezone').val(),
							availability: days
						});
