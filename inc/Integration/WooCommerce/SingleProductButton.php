...
class SingleProductButton {
	...
	public function get_attendant_status_html( $settings ) {
		...
		if( $settings['status'] ) {
			...
			$status_text = Translator::translate_string( __( 'Online!', 'tmw-whatsapp' ), 'status_online' );
		} else {
			$classes[] = 'tmw-whatsapp-elementor-title-status-offline';
			$status_text = Translator::translate_string( __( 'Offline!', 'tmw-whatsapp' ), 'status_offline' );
		}
